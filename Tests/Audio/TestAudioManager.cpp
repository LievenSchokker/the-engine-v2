...
TEST(AudioManagerTest, LoadMusicCallsBackend)
{
    ...
    // loadMusic does NOT call playMusic → so this stays default
    EXPECT_EQ(ptr->lastPlayMusic_path, "");
    EXPECT_EQ(ptr->lastPlayMusic_loops, -1);
}
...
TEST(AudioManagerTest, PlaySound_UsesReserveChannelAndBackendPlay)
{
    ...
    // Load succeeds (no handles anymore)
    EXPECT_TRUE(mgr.loadSound("hit.wav"));

    // freeChannel is the fake backend's "next channel"
    int channel = mgr.playSound("hit.wav", 0, 1.0f, 0.5f);

    EXPECT_EQ(channel, ptr->freeChannel);

    EXPECT_EQ(ptr->lastPlaySound_path, "hit.wav");
    EXPECT_EQ(ptr->lastPlaySound_channel, ptr->freeChannel);
    EXPECT_EQ(ptr->lastPlaySound_loops, 0);

    EXPECT_FLOAT_EQ(ptr->lastPanLeft, 1.0f);
    EXPECT_FLOAT_EQ(ptr->lastPanRight, 0.5f);
}
