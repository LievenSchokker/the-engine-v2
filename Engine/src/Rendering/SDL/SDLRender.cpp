...
constexpr double kRotationThresholdDegrees = 0.01; // Threshold below which rotation is treated as zero
...
void SDLRender::open(const WindowOptions &opts)
{
    ...
    {
        std::cerr << "Invalid window dimensions: " << opts.width << "x" << opts.height << " (minimum: " << kMinWindowDimension << "x"
                  << kMinWindowDimension << ")\n";
        return;
    }
    ...
    window = SDL_CreateWindow(opts.title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, opts.width, opts.height, flags);
...
void SDLRender::drawCircle(const Vector2 &center, double radius, const Color &color, const Vector2 &scale)
{
    ...
    for (int y = -ry; y <= ry; ++y)
    {
        const double normalizedY = static_cast<double>(y) / static_cast<double>(ry);
        const double span = static_cast<double>(rx) * std::sqrt(std::max(0.0, 1.0 - normalizedY * normalizedY));
        const int startX = static_cast<int>(std::floor(-span));
        const int endX = static_cast<int>(std::ceil(span));
        SDL_RenderDrawLine(renderer, centerX + startX, centerY + y, centerX + endX, centerY + y);
    }
}
...
void SDLRender::drawRectangle(const Vector2 &center, const Vector2 &size, double rotationDegrees, const Color &color, const Vector2 &scale)
{
    ...
    {
        SDL_FRect rect{static_cast<float>(center.x - halfWidth), static_cast<float>(center.y - halfHeight), static_cast<float>(width),
    ...
    {
        const double rotatedX = corners[i].x * cosTheta - corners[i].y * sinTheta;
        const double rotatedY = corners[i].x * sinTheta + corners[i].y * cosTheta;
