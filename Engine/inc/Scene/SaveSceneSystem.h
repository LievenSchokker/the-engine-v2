...
public:
     ...
     */
    static std::vector<std::unique_ptr<Scene>> loadScenes(const std::string& filePath);
     ...
     */
    static std::string makeFilePath(const std::string& baseName, bool multiScene = false);
     ...
     * @return Complete file path with extension.
     */
    static std::string makeFilePath(const Scene& scene, const std::string& directory = "");
...
private:
    ...
    static bool writeToFile(const std::vector<std::byte>& bytes, const std::string& filePath);
