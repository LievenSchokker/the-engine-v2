...
struct Vector2
{
	...
	/// (1 point in the same direction; -1 complete opposite directions; 0 if
	/// perpendicular)
	...
	/// Linearly interpolates between @c from and @c to by @c t (t will be
	...
	/// Adds each element of the other Vector to this Vector's respective
	/// element (v1.x + v2.x, v1.y + v2.y)
	Vector2 operator+(const Vector2& other) const;

	/// Subtracts each element of the other Vector to this Vector's respective
	/// element (v1.x - v2.x, v1.y - v2.y)
	Vector2 operator-(const Vector2& other) const;

	/// Multiplies each element of the other Vector to this Vector's respective
	/// element (v1.x * v2.x, v1.y * v2.y)
	Vector2 operator*(const Vector2& other) const;

	/// Divides each element of the other Vector to this Vector's respective
	/// element (v1.x / v2.x, v1.y / v2.y)
	Vector2 operator/(const Vector2& other) const;

	/// Multiplies this vector's component by a scalar value (v1.x * scalar,
	/// v1.y * scalar)
	Vector2 operator*(float scalar) const;

	/// Divides this vector's component by a scalar value (v1.x / scalar, v1.y /
	/// scalar)
	Vector2 operator/(float scalar) const;
