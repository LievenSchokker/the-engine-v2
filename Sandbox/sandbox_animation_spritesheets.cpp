...
int main(int argc, char** argv)
{
	...
	bool playerLoaded = SpritesheetLoader::loadSpritesheet(
		...
		if ( playerLoaded )
		{
			...
			// Add player controller behaviour for movement and animation
			// switching
		...
		if ( ratLoaded )
		{
			...
			// FlipX tracks: face right during forward movement, left during
			// reverse Forward flip track (0 to ratMoveDuration): facing right
			// (FlipX = 0)
